...
这里有一个组内变量的数据集 (来自 Morey 2008): pre/post-test。
...
请注意，这并未更改 x 轴标签。 有关如何修改轴标签的信息，请参见[坐标轴 (ggplot2)](#ggplot2-axes)。
...
如果你希望不同的线条出现在不同的方面，有两个选项。 一种是创建具有所需线条值的新数据框。 另一种选择（控制更有限）是在 `geom_line（）` 中使用 `stat` 和 `xintercept`。
...
你还可以使用其他颜色标度，例如从 RColorBrewer 包中获取。 请参阅下面的 RColorBrewer 调色板图表。
