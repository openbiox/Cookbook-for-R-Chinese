...
这个函数一定程度上使用了[暴力算法](http://www-igm.univ-mlv.fr/~lecroq/string/node3.html)来生成每个拉丁方，有时候它会因为没有可用的数字放入给定的位置而失败。这种情况下，它会再做尝试。可能存在一种更好的办法吧，但我并不清楚。
...
# 因此我们这里直接硬编码值 (来自 http://oeis.org/A002860)
